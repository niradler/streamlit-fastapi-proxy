...
def list_apps():
    ...
    for app in app_service.registry.get_all():
        ...
        if is_running:
            ...
            app_info["external_process"] = app_service.running[app.slug].get('external_process', False)
...
async def start_app(slug: str):
    ...
    if app_service.is_app_running(slug):
        ...
        return {
            ...
            "port": port,
            "external_process": app_service.running[slug].get('external_process', False)
        }
...
async def cleanup_all_apps():
    ...
    return {
        ...
        "total_stopped": len(app_service.running)
    }
