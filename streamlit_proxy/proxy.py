...
class StreamlitWebSocketProxy:
    ...
    async def proxy_websocket(
        self, client_ws: WebSocket, target_url: str, slug: str, endpoint_name: str = ""
    ):
        ...
        logger.info(
            f"{endpoint_name} WebSocket connection for slug '{slug}' -> {target_url}"
        )
        ...
        try:
            ...
            logger.error(
                f"Failed to accept {endpoint_name} WebSocket for '{slug}': {e}"
            )
            return
        ...
        while retry_count < self.max_retries:
            try:
                await self._proxy_session(
                    client_ws, target_url, slug, endpoint_name, stats
                )
                ...
                if retry_count < self.max_retries:
                    ...
                    logger.warning(
                        f"Reconnecting in {wait_time}s after connection error: {e}"
                    )
                    await asyncio.sleep(wait_time)
                else:
                    logger.error(f"Max retries reached for {connection_id}")
                    await client_ws.close(
                        code=1002, reason="Cannot connect to upstream"
                    )
                    break
    ...
    async def _proxy_session(
        ...
        async with aconnect_ws(
            ...
            * 1024
            * 1024,  # 10MB for large Streamlit dataframes
        ) as backend_ws:
            ...
            while True:
                try:
                    ...
                    if hasattr(message, "data") and message.data is not None:
                        ...
                        logger.debug(
                            f"[{slug}] Backend->Client: {len(message.text)} chars (as bytes)"
                        )
                    else:
    ...
    async def _forward_with_monitoring(
        self, source_ws, target_ws, direction: str, stats: ConnectionStats
    ):
...
@router.api_route(
    "/{slug}/", methods=["GET", "POST", "PUT", "DELETE", "PATCH", "HEAD", "OPTIONS"]
)
...
async def proxy_handler(slug: str, path: str, request: Request):
    ...
    if not app_service.is_app_running(slug):
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to start app: {str(e)}"
            )
    ...
    try:
        ...
        raise HTTPException(
            status_code=502, detail=f"Could not connect to app on port {port}"
        )
    except httpx.TimeoutException as e:
        logger.error(f"Timeout error to {target_url}: {str(e)}")
        raise HTTPException(
            status_code=504, detail=f"Timeout connecting to app on port {port}"
        )
...
async def websocket_proxy_with_httpx(
    websocket: WebSocket, target_url: str, slug: str, endpoint_name: str = ""
):
    """Enhanced WebSocket proxy using the robust StreamlitWebSocketProxy."""
    await websocket_proxy_instance.proxy_websocket(
        websocket, target_url, slug, endpoint_name
    )
...
async def streamlit_websocket_proxy(websocket: WebSocket, slug: str):
    ...
    if not app_service.is_app_running(slug):
        ...
        try:
            ...
            logger.error(
                f"Failed to start app '{slug}' for Streamlit WebSocket: {str(e)}"
            )
            await websocket.close(code=1008, reason="Failed to start app")
            return
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as test_client:
                ...
                if test_response.status_code == 200:
                    logger.info(
                        f"Streamlit health check passed: {test_response.status_code}"
                    )
                    health_check_passed = True
                    break
                else:
                    logger.warning(
                        f"Streamlit health check returned {test_response.status_code}"
                    )
        except Exception as e:
    ...
    if not health_check_passed:
        logger.warning(
            f"Streamlit health check failed for '{slug}', but proceeding with WebSocket connection"
        )
...
async def generic_websocket_proxy(websocket: WebSocket, slug: str, ws_path: str):
    ...
    if not any(
        ws_indicator in ws_path.lower()
        for ws_indicator in ["stream", "websocket", "ws", "_stcore"]
    ):
    ...
    if not app_service.is_app_running(slug):
        ...
        try:
            ...
            logger.error(
                f"Failed to start app '{slug}' for generic WebSocket: {str(e)}"
            )
            await websocket.close(code=1008, reason="Failed to start app")
            return
